---
title: "Assess the impact of TF, using simulated individual-level data based on WTCCC & Liver-specific network"
author: "Xiang Zhu"
date: "2018-09-19"
...
viz_res <- function(pip,pve) {
  res <- R.matlab::readMat(paste0(data_path,"wtccc_ind_liver_pip",100*pip,"pve",100*pve,"_seed644_assess_tf.mat"))
  ...
  sp <- sp + geom_abline(slope=1, intercept=0, color="red") + ggtitle("Impact of TF on marginal likelihood")
  ...
  hp <- hp + geom_hline(yintercept=res$pve, color="red") + ggtitle("Impact of TF on PVE")
  
  pp <- ggpubr::ggboxplot(est_df, x="model", y="pip", xlab="Model", ylab="Estimated PIP") 
  pp <- pp + geom_hline(yintercept=res$pip, color="red") + ggtitle("Impact of TF on PIP")
