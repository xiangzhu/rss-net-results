---
title: "Supplementary Figure 14 of Zhu et al (2020)"
author: "Xiang Zhu"
date: "2020-12-02"
...
make_df <- function(proj_path,gwas_name) {
  ...
  rc_file <- paste0(proj_path,"rc_model_results/aggregated_results/",gwas_name,"_model.rds")

  oe_df <- readRDS(oe_file)
  rc_df <- readRDS(rc_file)
  
  names(oe_df) <- paste0("oe_",names(oe_df))
  names(rc_df) <- paste0("rc_",names(rc_df))

  all_df <- dplyr::inner_join(rc_df[,c("rc_net","rc_bf40")],oe_df[,c("oe_net","oe_bf40")],by=c("rc_net"="oe_net"))
  
  all_df$log10_oe_bf40 <- log10(all_df$oe_bf40)
  all_df$log10_rc_bf40 <- log10(all_df$rc_bf40)
  
  all_df <- all_df[,c("rc_net","log10_oe_bf40","log10_rc_bf40")]
  
  all_df <- dplyr::arrange(all_df,-log10_oe_bf40)
  
  names(all_df)[names(all_df)=="rc_net"] <- "Network"
  names(all_df)[names(all_df)=="log10_oe_bf40"] <- "<i>y</i>-axis"
  names(all_df)[names(all_df)=="log10_rc_bf40"] <- "<i>x</i>-axis"
...
Supplementary Figure 14 of [Zhu et al (2020)][].
...
```{r, echo=FALSE}
my_df <- make_df(proj_path,gwas_name="uc2015")
...
```{r, echo=FALSE}
my_df <- make_df(proj_path,gwas_name="load2013")
