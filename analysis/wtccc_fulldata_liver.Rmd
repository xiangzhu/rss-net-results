...
viz_res <- function(pip,pve) {
  ...
  # viz whole-genome gene-level results
  df <- data.frame(
  ...
  roc_df <- data.frame(D=ifelse(c(res$gamma[tgtf_nid])!=0, 1, 0), M=c(res$alpha[tgtf_nid]))
  ...
  roc_df <- data.frame(D=ifelse(c(res$gamma[tg_nid])!=0, 1, 0), M=c(res$alpha[tg_nid]))
  ...
  roc_df <- data.frame(D=ifelse(c(res$gamma[tf_nid])!=0, 1, 0), M=c(res$alpha[tf_nid]))
