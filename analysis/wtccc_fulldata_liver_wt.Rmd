---
title: "Assess the impact of TF-TG scores, using simulated individual-level data based on WTCCC & Liver-specific network"
author: "Xiang Zhu"
date: "2018-09-19"
...
viz_res <- function(pip,pve) {
  res <- R.matlab::readMat(paste0(data_path,"wtccc_ind_liver_pip",100*pip,"pve",100*pve,"_seed644_assess_wt.mat"))
  ...
  sp <- sp + geom_abline(slope=1, intercept=0, color="red") + ggtitle("Impact of TF-TG scores on marginal likelihood")
  ...
  hp <- hp + geom_hline(yintercept=res$pve, color="red") + ggtitle("Impact of TF-TG scores on PVE")
  
  pp <- ggpubr::ggboxplot(est_df, x="model", y="pip", xlab="Model", ylab="Estimated PIP") 
  pp <- pp + geom_hline(yintercept=res$pip, color="red") + ggtitle("Impact of TF-TG scores on PIP")

  print(ggarrange(sp, hp, pp, ncol=3))
}
```

## PIP: `r pip=0.1; pip` & PVE: `r pve=0.2; pve` (still running)

```{r, fig.width=15, fig.height=5, echo=FALSE, eval=FALSE}
viz_res(pip,pve)
```

## PIP: `r pip=0.2; pip` & PVE: `r pve=0.2; pve` (still running)
